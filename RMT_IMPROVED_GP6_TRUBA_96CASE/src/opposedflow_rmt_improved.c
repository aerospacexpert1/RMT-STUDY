/*
  opposedflow_rmt_improved.c

  Experimental improved Robust Multigrid Technique (RMT) pressure solver for the
  structured-grid opposed-flow reacting-flow code used in the thesis campaign.

  IMPORTANT PROVENANCE:
    - reacting-flow physics, transport, chemistry, time stepping, pressure RHS,
      velocity/flux correction, boundary conditions and output are inherited from
      the frozen V95/OpenFOAM-matched production source;
    - only the pressure linear solver is replaced.

  RMT_IMPROVED_GP6 changes relative to the earlier production RMT line:
    - factor-three 3x3 shifted coarse-grid families;
    - sawtooth cycle (no presmoothing);
    - six post-smoothing RBGS sweeps by default;
    - shift-aware homogeneous pressure-correction boundary treatment;
    - geometric OpenMP task parallelism through configurable shifted-grid depth;
    - algebraic OpenMP RBGS smoothing on the finest correction grid;
    - small-grid CG coarse solve with strict residual tolerance;
    - full correction attempted first, with monotone backtracking safeguard;
    - explicit diagnostics for coarse solves, task creation, backtracking and
      emergency fine-grid RBGS safeguards.

  This file is an experimental additional campaign.  It does not replace or
  overwrite the thesis baseline RMT_FINALVOL2 results.
  ...
  Compile:
    gcc -O3 -march=native -std=c11 -Wall -Wextra -Wpedantic -fopenmp \
        opposedflow_rmt_improved.c -lm -o opposedflow_rmt_improved

  Example:
    OMP_NUM_THREADS=4 ./opposedflow_rmt_improved \
...
typedef struct {
    ...
    /* Legacy mg* controls retained for CLI compatibility with the frozen V95 source. */
    int mgPreSmooth;
    int mgPostSmooth;
    int mgCoarseSweeps;
    double mgOmega;

    /* Improved RMT pressure controls. */
    int rmtMaxLevels;
    int rmtPostSmooth;
    int rmtCoarseMaxIters;
    int rmtTaskDepth;
    int rmtMaxBacktracks;
    double rmtCoarseRelTol;
    double rmtOmegaStart;
...
static void init_case(Grid *g, Phys *ph, Controls *c) {
    ...
    c->poissonIters = 500; c->scalarIters = 4; c->scalarSweepsPerCycle = 5;
    c->mgPreSmooth = 3;
    c->mgPostSmooth = 3;
    c->mgCoarseSweeps = 40;
    c->mgOmega = 1.0;

    /* Fixed RMT_IMPROVED_GP6 production defaults. */
    c->rmtMaxLevels = 8;
    c->rmtPostSmooth = 6;
    c->rmtCoarseMaxIters = 64;
    c->rmtTaskDepth = 2;
    c->rmtMaxBacktracks = 10;
    c->rmtCoarseRelTol = 1.0e-11;
    c->rmtOmegaStart = 1.0;
...
static void parse_args(int argc, char **argv, Grid *g, Phys *ph, Controls *c) {
    for (int i=1; i<argc; ++i) {
        ...
        else if (!strcmp(argv[i], "-rmtCoarseIters") && i+1<argc) c->rmtCoarseMaxIters = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-rmtTaskDepth") && i+1<argc) c->rmtTaskDepth = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-rmtBacktracks") && i+1<argc) c->rmtMaxBacktracks = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-rmtCoarseRelTol") && i+1<argc) c->rmtCoarseRelTol = atof(argv[++i]);
        else if (!strcmp(argv[i], "-rmtOmegaStart") && i+1<argc) c->rmtOmegaStart = atof(argv[++i]);
    ...
    if (c->rmtMaxLevels < 2 || c->rmtMaxLevels > 12) die("rmtLevels must be in [2,12]");
    if (c->rmtPostSmooth < 1 || c->rmtPostSmooth > 32) die("rmtPost must be in [1,32]");
    if (c->rmtCoarseMaxIters < 1) die("rmtCoarseIters must be positive");
    if (c->rmtTaskDepth < 0 || c->rmtTaskDepth > c->rmtMaxLevels) die("invalid rmtTaskDepth");
    if (c->rmtMaxBacktracks < 0 || c->rmtMaxBacktracks > 20) die("rmtBacktracks must be in [0,20]");
    if (!(c->rmtCoarseRelTol > 0.0 && c->rmtCoarseRelTol < 1.0)) die("rmtCoarseRelTol must be in (0,1)");
    if (!(c->rmtOmegaStart > 0.0 && c->rmtOmegaStart <= 2.0)) die("rmtOmegaStart must be in (0,2]");
...
/* ------------------------------------------------------------------------- */
/* RMT_IMPROVED_GP6 pressure-correction kernel                               */
/* ------------------------------------------------------------------------- */
...
static long long g_rmt_shift_branches = 0;
static long long g_rmt_tasks_spawned = 0;
static long long g_rmt_coarse_solves = 0;
static long long g_rmt_coarse_cg_iters = 0;
static long long g_rmt_point_updates = 0;
static long long g_rmt_backtracks = 0;
static long long g_rmt_fallback_rbgs_sweeps = 0;

static void rmt_diag_add_ll(long long *dst, long long value) {
    #pragma omp atomic update
    *dst += value;
}

static int rmt_shift_count(int N, int shift) {
    const int first = shift + 1;
    if (first > N) return 0;
    return 1 + (N - first)/3;
}

static RMTGridMeta rmt_root_meta(const Grid *g) {
    RMTGridMeta m;
    m.g = *g;
    m.xFirst = 0.5*g->dx;
    m.xLast  = g->Lx - 0.5*g->dx;
    m.yFirst = 0.5*g->dy;
    m.yLast  = g->Ly - 0.5*g->dy;
    return m;
}

static RMTGridMeta rmt_child_meta(const RMTGridMeta *p, int sx, int sy) {
    RMTGridMeta c;
    c.g = p->g;
    c.g.Nx = rmt_shift_count(p->g.Nx, sx);
    c.g.Ny = rmt_shift_count(p->g.Ny, sy);
    c.g.dx = 3.0*p->g.dx;
    c.g.dy = 3.0*p->g.dy;

    c.xFirst = p->xFirst + (double)sx*p->g.dx;
    c.yFirst = p->yFirst + (double)sy*p->g.dy;
    c.xLast = c.xFirst + (double)MAX(0,c.g.Nx-1)*c.g.dx;
    c.yLast = c.yFirst + (double)MAX(0,c.g.Ny-1)*c.g.dy;
    return c;
}

/*
 * Homogeneous pressure-correction boundary condition.
 *
 * At the two side patches the physical pressure value is fixed during one
 * pressure solve, therefore the correction is zero at the physical boundary.
 * A shifted coarse grid does not in general place its first cell centre at H/2.
 * Linear continuation through the physical boundary gives
 *
 *     c_g = -(1-xi)/xi * c_1,   xi = d/H,
 *
 * where d is the distance of the first/last shifted cell centre from the
 * boundary.  For the finest grid xi=1/2 and this reduces exactly to c_g=-c_1.
 *
 * Bottom/top pressure patches are zeroGradient, so the correction uses the
 * corresponding homogeneous Neumann relation c_g=c_1.
 */
static void rmt_apply_correction_bc(const RMTGridMeta *m, double *x) {
    const int nx=m->g.Nx, ny=m->g.Ny;
    const double eps=1.0e-12;

    double xiL = m->xFirst/MAX(m->g.dx,eps);
    double xiR = (m->g.Lx-m->xLast)/MAX(m->g.dx,eps);
    xiL = MIN(1.0,MAX(1.0e-8,xiL));
    xiR = MIN(1.0,MAX(1.0e-8,xiR));

    const double aL = -(1.0-xiL)/xiL;
    const double aR = -(1.0-xiR)/xiR;

    for (int j=1; j<=ny; ++j) {
        x[IDX(0,j,ny)]    = aL*x[IDX(1,j,ny)];
        x[IDX(nx+1,j,ny)] = aR*x[IDX(nx,j,ny)];
    }
    for (int i=1; i<=nx; ++i) {
        x[IDX(i,0,ny)]    = x[IDX(i,1,ny)];
        x[IDX(i,ny+1,ny)] = x[IDX(i,ny,ny)];
    }

    x[IDX(0,0,ny)]       = x[IDX(1,1,ny)];
    x[IDX(0,ny+1,ny)]    = x[IDX(1,ny,ny)];
    x[IDX(nx+1,0,ny)]    = x[IDX(nx,1,ny)];
    x[IDX(nx+1,ny+1,ny)] = x[IDX(nx,ny,ny)];
}

static void rmt_apply_A(const RMTGridMeta *m, double *x, double *Ax) {
    const int nx=m->g.Nx, ny=m->g.Ny;
    const double ax=1.0/(m->g.dx*m->g.dx);
    const double ay=1.0/(m->g.dy*m->g.dy);
    const double ap=2.0*ax+2.0*ay;

    rmt_apply_correction_bc(m,x);
    for (int i=1;i<=nx;++i) for (int j=1;j<=ny;++j) {
        Ax[IDX(i,j,ny)] =
            ap*x[IDX(i,j,ny)]
          - ax*(x[IDX(i+1,j,ny)] + x[IDX(i-1,j,ny)])
          - ay*(x[IDX(i,j+1,ny)] + x[IDX(i,j-1,ny)]);
    }
}

static double rmt_dot_interior(const RMTGridMeta *m, const double *a, const double *b) {
    double s=0.0;
    const int nx=m->g.Nx, ny=m->g.Ny;
    for (int i=1;i<=nx;++i) for (int j=1;j<=ny;++j)
        s += a[IDX(i,j,ny)]*b[IDX(i,j,ny)];
    return s;
}

static void rmt_rbgs_serial(const RMTGridMeta *m, double *x, const double *b, int sweeps) {
    const int nx=m->g.Nx, ny=m->g.Ny;
    const double ax=1.0/(m->g.dx*m->g.dx);
    const double ay=1.0/(m->g.dy*m->g.dy);
    const double ap=2.0*ax+2.0*ay;
        ...
        for (int color=0;color<2;++color) {
            ...
            for (int i=1;i<=nx;++i) for (int j=1;j<=ny;++j) {
                ...
                x[IDX(i,j,ny)] =
                    (b[IDX(i,j,ny)]
                     + ax*(x[IDX(i+1,j,ny)] + x[IDX(i-1,j,ny)])
                     + ay*(x[IDX(i,j+1,ny)] + x[IDX(i,j-1,ny)]))/ap;
            }
        }
    }
    rmt_apply_correction_bc(m,x);
    rmt_diag_add_ll(&g_rmt_point_updates,(long long)nx*(long long)ny*(long long)sweeps);
}

/* Algebraic parallelism on the finest correction grid. */
static void rmt_rbgs_parallel_root(const RMTGridMeta *m, double *x, const double *b, int sweeps) {
    const int nx=m->g.Nx, ny=m->g.Ny;
    const double ax=1.0/(m->g.dx*m->g.dx);
    const double ay=1.0/(m->g.dy*m->g.dy);
    const double ap=2.0*ax+2.0*ay;
...
/* Control-volume averaging of the fine-grid residual density onto one shift. */
static void rmt_restrict_shifted(const RMTGridMeta *p, int sx, int sy,
                                 const double *bf, const RMTGridMeta *c, double *bc) {
    const int nyf=p->g.Ny, nyc=c->g.Ny;

    for (int ic=1; ic<=c->g.Nx; ++ic) {
        const int i0=sx+1+3*(ic-1);
        for (int jc=1; jc<=c->g.Ny; ++jc) {
            const int j0=sy+1+3*(jc-1);
            double sum=0.0;
            int count=0;
            for (int i=MAX(1,i0-1); i<=MIN(p->g.Nx,i0+1); ++i) {
                for (int j=MAX(1,j0-1); j<=MIN(p->g.Ny,j0+1); ++j) {
                    sum += bf[IDX(i,j,nyf)];
                    ++count;
                }
            }
            bc[IDX(ic,jc,nyc)] = count ? sum/(double)count : 0.0;
        }
    }
}

/*
 * RMT prolongation is an index mapping/injection.  The nine 2-D shifted
 * families occupy different (i mod 3,j mod 3) residue classes, so sibling
 * injections are disjoint and can be performed by independent OpenMP tasks.
 */
static void rmt_inject_shifted(const RMTGridMeta *p, int sx, int sy,
                               double *xf, const RMTGridMeta *c, const double *xc) {
    const int nyf=p->g.Ny, nyc=c->g.Ny;
    for (int ic=1; ic<=c->g.Nx; ++ic) {
        const int i0=sx+1+3*(ic-1);
        for (int jc=1; jc<=c->g.Ny; ++jc) {
            const int j0=sy+1+3*(jc-1);
            xf[IDX(i0,j0,nyf)] += xc[IDX(ic,jc,nyc)];
        }
    }
}

/*
 * Small-grid solve.  Matrix-free CG is used as an accurate "direct-like"
 * terminal treatment.  With the default depth the terminal shifted systems
 * contain only a few unknowns, so this avoids a fixed large number of coarse
 * relaxation sweeps while retaining a strict residual target.
 */
static void rmt_coarse_solve_cg(const RMTGridMeta *m, double *x, const double *b,
                                const Controls *c) {
    const size_t n=(size_t)(m->g.Nx+2)*(size_t)(m->g.Ny+2);
    double *r=(double*)calloc(n,sizeof(double));
    double *p=(double*)calloc(n,sizeof(double));
    double *Ap=(double*)calloc(n,sizeof(double));
    if(!r || !p || !Ap) die("allocate RMT coarse CG workspace");

    rmt_diag_add_ll(&g_rmt_coarse_solves,1);

    rmt_apply_A(m,x,Ap);
    for(int i=1;i<=m->g.Nx;++i) for(int j=1;j<=m->g.Ny;++j) {
        const size_t k=(size_t)IDX(i,j,m->g.Ny);
        r[k]=b[k]-Ap[k];
        p[k]=r[k];
    }

    const double b2=MAX(rmt_dot_interior(m,b,b),1.0e-60);
    double rr=rmt_dot_interior(m,r,r);
    const double target2=c->rmtCoarseRelTol*c->rmtCoarseRelTol*b2;
    int iters=0;

    for(int it=0; it<c->rmtCoarseMaxIters && rr>target2; ++it) {
        rmt_apply_A(m,p,Ap);
        const double pAp=rmt_dot_interior(m,p,Ap);
        if(!isfinite(pAp) || pAp<=1.0e-60) break;
        ...
        const double rrNew=rmt_dot_interior(m,r,r);
        ++iters;
        if(!isfinite(rrNew) || rrNew<=target2) {
            rr=rrNew;
            break;
        }

        const double beta=rrNew/MAX(rr,1.0e-60);
        for(int i=1;i<=m->g.Nx;++i) for(int j=1;j<=m->g.Ny;++j) {
            const size_t k=(size_t)IDX(i,j,m->g.Ny);
            p[k]=r[k]+beta*p[k];
        }
    ...
    rmt_apply_correction_bc(m,x);
    rmt_diag_add_ll(&g_rmt_coarse_cg_iters,iters);

    free(r); free(p); free(Ap);
}

static void rmt_recursive_sawtooth(const RMTGridMeta *m, double *x, const double *b,
                                   const Controls *c, int level);

static void rmt_process_child(const RMTGridMeta *parent, double *parentX,
                              const double *parentB, const Controls *c,
                              int level, int sx, int sy) {
    const RMTGridMeta child=rmt_child_meta(parent,sx,sy);
    if(child.g.Nx<1 || child.g.Ny<1) return;

    const size_t nc=(size_t)(child.g.Nx+2)*(size_t)(child.g.Ny+2);
    double *bc=(double*)calloc(nc,sizeof(double));
    double *xc=(double*)calloc(nc,sizeof(double));
    if(!bc || !xc) die("allocate RMT shifted-grid workspace");

    rmt_diag_add_ll(&g_rmt_shift_branches,1);
    rmt_restrict_shifted(parent,sx,sy,parentB,&child,bc);
    rmt_recursive_sawtooth(&child,xc,bc,c,level);
    rmt_inject_shifted(parent,sx,sy,parentX,&child,xc);
...
static void rmt_recursive_sawtooth(const RMTGridMeta *m, double *x, const double *b,
                                   const Controls *c, int level) {
    const int terminal =
        (level+1>=c->rmtMaxLevels) ||
        (m->g.Nx<=2 && m->g.Ny<=2);

    if(terminal) {
        rmt_coarse_solve_cg(m,x,b,c);
        return;
    }
    ...
    for(int sx=0;sx<3;++sx) for(int sy=0;sy<3;++sy) {
        ...
        if(child.g.Nx<1 || child.g.Ny<1) continue;

        if(spawnTasks) {
            rmt_diag_add_ll(&g_rmt_tasks_spawned,1);
            #pragma omp task firstprivate(sx,sy,level) shared(x,b)
            {
                rmt_process_child(m,x,b,c,level+1,sx,sy);
            }
        } else {
            rmt_process_child(m,x,b,c,level+1,sx,sy);
        }
    ...
    /*
     * Sawtooth RMT: no presmoothing.  On every non-root level the assembled
     * injected correction is post-smoothed locally.  The root is smoothed
     * after the geometric task region so all threads can participate.
     */
    if(level>0)
        rmt_rbgs_serial(m,x,b,c->rmtPostSmooth);
}

static void rmt_build_correction(const Grid *g, double *corr, const double *residual,
                                 const Controls *c) {
    const RMTGridMeta root=rmt_root_meta(g);
    const size_t n=(size_t)(g->Nx+2)*(size_t)(g->Ny+2);
    memset(corr,0,n*sizeof(double));

    /*
     * Geometric parallelism: independent shifted-grid branches are OpenMP
     * tasks.  Descendant tasks are generated down to rmtTaskDepth.
     */
    #pragma omp parallel
    {
        #pragma omp single
        {
            rmt_recursive_sawtooth(&root,corr,residual,c,0);
        }
    }

    /* Algebraic parallelism on the finest correction grid. */
    rmt_rbgs_parallel_root(&root,corr,residual,c->rmtPostSmooth);
}

/* ------------------------------------------------------------------------- */
/* End RMT_IMPROVED_GP6 kernel                                               */
/* ------------------------------------------------------------------------- */
...
static PressureStats solve_pressure_poisson(const Grid *g, const Phys *ph, Fields *f,
    ...
    if(!corr) die("allocate RMT fine correction");
    ...
    for(int cyc=0;cyc<c->poissonIters;++cyc) {
        /*
         * f->work contains the current fine-grid residual r=b-Ap.
         * Build one sawtooth multiple-shift RMT approximation of A c = r.
         */
        rmt_build_correction(g,corr,f->work,c);

        const double oldResidual=stats.absResidual;
        double omega=c->rmtOmegaStart;
        int accepted=0;

        for(int bt=0;bt<=c->rmtMaxBacktracks;++bt) {
            #pragma omp parallel for collapse(2) schedule(static)
            for(int i=1;i<=g->Nx;++i) for(int j=1;j<=g->Ny;++j)
                f->p[IDX(i,j,g->Ny)] += omega*corr[IDX(i,j,g->Ny)];

            apply_bc_velocity_pressure(g,ph,f);
            pressure_residual(g,f,f->rhs,f->work);
            const double trial=array_max_abs_interior(g,f->work);

            if(isfinite(trial) && trial<oldResidual) {
                stats.absResidual=trial;
                accepted=1;
                break;
            }
            ...
            apply_bc_velocity_pressure(g,ph,f);
            omega*=0.5;
            rmt_diag_add_ll(&g_rmt_backtracks,1);
        }

        /*
         * Rare safety net only.  It is counted explicitly so the campaign can
         * demonstrate whether the improved RMT correction is self-sufficient.
         * A zero value of diag_fallback_rbgs_sweeps means every RMT cycle was
         * accepted without this safeguard.
         */
        if(!accepted) {
            rbgs_pressure_sweeps(g,ph,f,f->rhs,1,1.0);
            pressure_residual(g,f,f->rhs,f->work);
            stats.absResidual=array_max_abs_interior(g,f->work);
            rmt_diag_add_ll(&g_rmt_fallback_rbgs_sweeps,1);
        }
        ...
        /*
         * pressure_residual() above already refreshed f->work for an accepted
         * correction or the fallback sweep, so the next RMT cycle starts from
         * the correct residual without another fine-grid pass.
         */
    }
...
static void write_pressure_step_history(const Controls *c, const PressureStepHistory *h) {
    ...
    fprintf(fp, "step,time_s,dt_s,rmt_cycles,initial_residual,final_residual,final_relative_residual,convergence_factor\n");
...
static void write_final_cycle_history(const Controls *c, const CycleHistory *h) {
    ...
    fprintf(fp, "cycle,absolute_residual,relative_residual,residual_ratio_to_previous\n");
...
static void write_case_summary_csv(const Grid *g, const Phys *ph, const Controls *c,
                                   ...
                                   long long pressureSolves, long long totalPressureCycles,
                                   int maxPressureCycles, double meanConvergenceFactor,
    ...
    const double avgCycles=pressureSolves>0
        ? (double)totalPressureCycles/(double)pressureSolves : 0.0;
    const double pressureFraction=computeElapsed>0.0
        ? 100.0*pressureElapsed/computeElapsed : 0.0;
    const double cells=(double)g->Nx*(double)g->Ny;
    const double equivSweeps=cells>0.0 ? (double)g_rmt_point_updates/cells : 0.0;
    const double equivSweepsPerSolve=pressureSolves>0
        ? equivSweeps/(double)pressureSolves : 0.0;
    ...
    fprintf(fp,
        "case_id,solver,threads,Nx,Ny,cells,level0_Nx,level0_Ny,"
        "rmt_smoothing_sweeps,rmt_max_cycles,rmt_max_levels,rmt_task_depth,"
        "rmt_coarse_max_iters,rmt_coarse_rel_tol,rmt_omega_start,rmt_max_backtracks,"
        "final_time_s,time_steps,vFuel_m_per_s,vOx_m_per_s,A,beta,Ta_K,HfF_J_per_kg,"
        "rho_kg_per_m3,mu_Pa_s,cp_J_per_kgK,Pr,Sc,"
        "compute_time_s,pressure_time_s,pressure_fraction_pct,output_time_s,total_wall_s,"
        "pressure_solves,total_rmt_cycles,avg_rmt_cycles_per_pressure_solve,max_rmt_cycles_per_pressure_solve,"
        "mean_pressure_convergence_factor,final_pressure_cycles,final_pressure_rhs_norm,"
        "final_pressure_initial_residual,final_pressure_abs_residual,final_pressure_rel_residual,"
        "final_pressure_convergence_factor,equivalent_fine_grid_smoothing_sweeps,"
        "equivalent_fine_grid_smoothing_sweeps_per_pressure_solve,"
        "mass_residual_max,mass_residual_l2,mass_residual_integral,"
        "Tmin_K,Tmax_K,pmin_Pa,pmax_Pa,umin_m_per_s,umax_m_per_s,"
        "vmin_m_per_s,vmax_m_per_s,YFmax,YOmax,YPmax,YN2max,"
        "diag_shift_branches,diag_tasks_spawned,diag_coarse_solves,diag_coarse_cg_iters,"
        "diag_backtracks,diag_fallback_rbgs_sweeps,diag_point_updates\n");

    fprintf(fp,"%s,RMT_IMPROVED_GP6,%d,%d,%d,%lld,%d,%d,%d,%d,%d,%d,%d,%.17g,%.17g,%d,%.17g,%d",
            c->caseId,c->threads,g->Nx,g->Ny,
            (long long)g->Nx*(long long)g->Ny,g->Nx,g->Ny,
            c->rmtPostSmooth,c->poissonIters,c->rmtMaxLevels,c->rmtTaskDepth,
            c->rmtCoarseMaxIters,c->rmtCoarseRelTol,c->rmtOmegaStart,c->rmtMaxBacktracks,
            finalTime,steps);
    ...
    fprintf(fp,",%lld,%lld,%.17g,%d,%.17g,%d,%.17g,%.17g,%.17g,%.17g,%.17g",
            pressureSolves,totalPressureCycles,avgCycles,maxPressureCycles,
    ...
    fprintf(fp,",%.17g,%.17g",equivSweeps,equivSweepsPerSolve);
    fprintf(fp,",%.17g,%.17g,%.17g",mass->maxAbs,mass->l2,mass->integral);

    fprintf(fp,",%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g",
            Tmin,Tmax,pmin,pmax,umin,umax,vmin,vmax,YFmax,YOmax,YPmax,YN2max);

    fprintf(fp,",%lld,%lld,%lld,%lld,%lld,%lld,%lld\n",
            g_rmt_shift_branches,g_rmt_tasks_spawned,g_rmt_coarse_solves,
            g_rmt_coarse_cg_iters,g_rmt_backtracks,g_rmt_fallback_rbgs_sweeps,
            g_rmt_point_updates);
...
int main(int argc, char **argv) {
    ...
    printf("RMT_IMPROVED_GP6 reacting-flow solver: frozen V95 physics, improved RMT pressure solve\n");
    printf("Case ID: %s\n", c.caseId);
    printf("Pressure grid: %dx%d, factor-three 3x3 shifted families\n", g.Nx, g.Ny);
    printf("RMT: post=%d maxCycles=%d levels=%d taskDepth=%d coarseMaxIters=%d coarseRelTol=%.3e omegaStart=%g backtracks=%d threads=%d\n",
           c.rmtPostSmooth,c.poissonIters,c.rmtMaxLevels,c.rmtTaskDepth,
           c.rmtCoarseMaxIters,c.rmtCoarseRelTol,c.rmtOmegaStart,c.rmtMaxBacktracks,c.threads);
    ...
    printf("Pressure maxRmtCycles=%d relTol=%.3e absTol=%.3e\n",
    ...
    while (time < c.endTime - 1e-15) {
        ...
        if (progressNow) {
            ...
            printf("step=%7d t=%10.6f dt=%9.3e Co=%8.2e Di=%8.2e pCyc=%3d pRel=%9.2e rhoRMT=%7.4f massMax=%9.2e massL2=%9.2e Tmax=%9.2f YPmax=%8.5f YOmax=%8.5f YFmax=%8.5f\n",
    ...
    printf("Final pressure: cycles=%d initialResidual=%.6e absResidual=%.6e relResidual=%.6e rhsNorm=%.6e rhoRMT=%.6f\n",
           pStats.cycles,pStats.initialResidual,pStats.absResidual,pStats.relResidual,pStats.rhsNorm,pStats.convergenceFactor);
    printf("Pressure totals: solves=%lld cycles=%lld avgCycles=%.6f maxCycles=%d pressureTime=%.6f s meanRhoRMT=%.6f\n",
    ...
    printf("RMT diagnostics: shiftedBranches=%lld tasks=%lld coarseSolves=%lld coarseCGIters=%lld backtracks=%lld fallbackRBGSSweeps=%lld pointUpdates=%lld\n",
           g_rmt_shift_branches,g_rmt_tasks_spawned,g_rmt_coarse_solves,g_rmt_coarse_cg_iters,
           g_rmt_backtracks,g_rmt_fallback_rbgs_sweeps,g_rmt_point_updates);
